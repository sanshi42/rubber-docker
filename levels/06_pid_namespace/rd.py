#!/usr/bin/env python3
"""从零实现 Docker 工作坊 - Level 6：添加 PID namespace。

目标：让你的新 process 从 PID 1 开始 :)
"""
...
def contain(command, image_name, image_dir, container_id, container_dir):
    ...
    linux.unshare(linux.CLONE_NEWUTS)  # 切换到新的 UTS namespace
...
def run(image_name, image_dir, container_dir, command):
    ...
    # TODO: 切换到新的 PID namespace（使用 unshare）只会影响
    #       当前 process 的 children（因为无法修改正在运行的 process 的 PID），
    #       所以我们必须在这里 unshare，或者用 linux.clone() 替换 os.fork()
